...
def getChargingValues(Battery, Index):
    ...
    for iterator in Battery['Voltage_measured']:
        index.append(i)
        i += 1
    return [index, Battery['Voltage_measured'], Battery['Current_measured'], Battery['Temperature_measured'], Battery['Voltage_charge'], Battery['Current_charge'], Battery['Time']]
...
def getDischargingValues(Battery, Index):
    ...
    for iterator in Battery['Voltage_measured']:
        index.append(i)
        i += 1
    return [index, Battery['Voltage_measured'], Battery['Current_measured'], Battery['Temperature_measured'], Battery['Voltage_load'], Battery['Current_load'], Battery['Time']]
...
def getVoltageZeroTime(Battery):
    ...
    for Bat in Battery:
        if Bat['cycle'] == 'discharge':
            cycle.append(i)
            Zerodf = pd.DataFrame({"Voltage_load":Bat['data']['Voltage_load'], "Time":Bat['data']['Time']})
            idx = Zerodf[Zerodf["Voltage_load"] == 0.000].index
            Zerodf.drop(idx, inplace = True)            
            condition = (Zerodf.Voltage_load == 0.001)
            Zerodf.loc[condition, ['Time']] 
            temp.append((max(Zerodf['Time'])))
            i += 1
    return [cycle, temp]


def getDischargeVolt(Battery):
    cycle = []
    temp = []
    i = 1
    for Bat in Battery:
        if Bat['cycle'] == 'discharge':
            cycle.append(i)
            temp.append(max(Bat['data']['Voltage_load']))
            i += 1
    return [cycle, temp]
...
def getDataframe(Battery):
    ...
    l3 = getVoltageZeroTime(Battery)
    l4 = getDischargeVolt(Battery)
    l5 = getSOH(Battery)
    data = {'cycle':l[0],'capacity':l[1], 'max_discharge_temp':l1[1], 'max_charge_temp':l2[1], 'Time':l3[1],'Voltage_load':l4[1], 'SOH': l5}
...
def getSOH(Battery):
    BatteryCapacity = getBatteryCapacity(Battery)
    BatterySOH = []
    for i in BatteryCapacity[1]:
        SOH = i/max(BatteryCapacity[1])
        BatterySOH.append(SOH)
    return BatterySOH
